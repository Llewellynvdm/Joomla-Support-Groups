 ...
 */
class SupportgroupsModelArea extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_supportgroups.area';
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_supportgroups.area');
			}
		}
		$this->areavvvx = $item->id;
	...
	public function getVvxsupport_groups()
	{
		...
		// Filter by areavvvx global.
		$areavvvx = $this->areavvvx;
		if (is_numeric($areavvvx ))
		{
			$query->where('a.area = ' . (int) $areavvvx );
		}
		elseif (is_string($areavvvx))
		{
			$query->where('a.area = ' . $db->quote($areavvvx));
		}
		else
		{
			$query->where('a.area = -5');
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		if ($id != 0 && (!$user->authorise('area.edit.state', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.state', 'com_supportgroups')))
		{
		...
		if ($id != 0 && (!$user->authorise('area.edit.created_by', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.created_by', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('area.edit.created', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.created', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('area.edit.name', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.name', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		// Modify the form based on Edit Area Type access controls.
		if ($id != 0 && (!$user->authorise('area.edit.area_type', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.area_type', 'com_supportgroups')))
		{
			// Disable fields for display.
			...
			{
				// Disable fields while saving.
				$form->setFieldAttribute('area_type', 'filter', 'unset');
				// Disable fields while saving.
				$form->setFieldAttribute('area_type', 'required', 'false');
			}
		}
		// Modify the form based on Edit Region access controls.
		if ($id != 0 && (!$user->authorise('area.edit.region', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.region', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('area.edit.details', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.details', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		// Modify the form based on Edit Color access controls.
		if ($id != 0 && (!$user->authorise('area.edit.color', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.color', 'com_supportgroups')))
		{
			// Disable fields for display.
			...
			{
				// Disable fields while saving.
				$form->setFieldAttribute('color', 'filter', 'unset');
				// Disable fields while saving.
				$form->setFieldAttribute('color', 'required', 'false');
			}
		}
		// Modify the form based on Edit Alias access controls.
		if ($id != 0 && (!$user->authorise('area.edit.alias', 'com_supportgroups.area.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area.edit.alias', 'com_supportgroups')))
		{
			// Disable fields for display.
	...
	{
		return 'administrator/components/com_supportgroups/models/forms/area.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('area.delete', 'com_supportgroups.area.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('area.edit.state', 'com_supportgroups.area.' . (int) $recordId);
		...
		return $user->authorise('area.edit.state', 'com_supportgroups');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('area.edit', 'com_supportgroups.area.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('area.edit',  'com_supportgroups');
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_supportgroups.edit.area.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('area.create') && !$this->canDo->get('area.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('area.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('area.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('area.edit') && !$this->canDo->get('area.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('area.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('area', 'supportgroupsTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_SUPPORTGROUPS_AREA_SAVE_WARNING');
				}
