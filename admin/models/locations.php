 ...
 * Locations Model
 */
class SupportgroupsModelLocations extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.region LIKE '.$search.' OR g.name LIKE '.$search.')');
			}
		}
		...
		{
			$query->where('a.region = ' . $db->quote($db->escape($region, true)));
		}
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__supportgroups_location");
	...
	*/
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__supportgroups_location'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__supportgroups_location'))->set($fields)->where($conditions); 
