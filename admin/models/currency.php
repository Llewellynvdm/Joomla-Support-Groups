...
{    
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_supportgroups.currency';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'currency', $prefix = 'SupportgroupsTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_supportgroups.currency', 'currency', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id != 0 && (!$user->authorise('currency.edit.state', 'com_supportgroups.currency.' . (int) $id))
			|| ($id == 0 && !$user->authorise('currency.edit.state', 'com_supportgroups')))
		{
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('currency.delete', 'com_supportgroups.currency.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('currency.edit.state', 'com_supportgroups.currency.' . (int) $recordId);
		...
		return $user->authorise('currency.edit.state', 'com_supportgroups');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('currency.edit', 'com_supportgroups.currency.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('currency.edit',  'com_supportgroups');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__supportgroups_currency'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_supportgroups.edit.currency.data', array());
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('currency', 'supportgroupsTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_SUPPORTGROUPS_CURRENCY_SAVE_WARNING');
				}
