...
/**
 * Supportgroups Location Model
 */
class SupportgroupsModelLocation extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_supportgroups.location';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'location', $prefix = 'SupportgroupsTable', $config = array())
	{
	...
	{		// Get the form.
		$form = $this->loadForm('com_supportgroups.location', 'location', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id != 0 && (!$user->authorise('location.edit.name', 'com_supportgroups.location.' . (int) $id))
			|| ($id == 0 && !$user->authorise('location.edit.name', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('location.edit.region', 'com_supportgroups.location.' . (int) $id))
			|| ($id == 0 && !$user->authorise('location.edit.region', 'com_supportgroups')))
		{
			// Disable fields for display.
	...
	{
		return 'administrator/components/com_supportgroups/models/forms/location.js';
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__supportgroups_location'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_supportgroups.edit.location.data', array());
