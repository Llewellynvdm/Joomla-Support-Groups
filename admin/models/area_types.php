 ...
 * Area_types Model
 */
class SupportgroupsModelArea_types extends JModelList
{
	...
	public function getItems()
	{
		...
		if (SupportgroupsHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('area_type.access', 'com_supportgroups.area_type.' . (int) $item->id) && JFactory::getUser()->authorise('area_type.access', 'com_supportgroups'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__supportgroups_area_type");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__supportgroups_area_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__supportgroups_area_type'))->set($fields)->where($conditions); 
