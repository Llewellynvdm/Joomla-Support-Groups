...
class SupportgroupsModelImport extends JModelLegacy
{
	 ...
	 * @var        string
	 */
	protected $_context = 'com_supportgroups.import';
	...
	protected function populateState()
	{
		...
		$this->setState('message', $app->getUserState('com_supportgroups.message'));
	...
	public function import()
	{
		...
		if ($package === null)
		{
			switch ($this->getType)
			{
				...
				default:
					$app->setUserState('com_supportgroups.message', JText::_('COM_SUPPORTGROUPS_IMPORT_NO_IMPORT_TYPE_FOUND'));
		...
		if (!$package || !$package['type'])
		{
			...
			$app->setUserState('com_supportgroups.message', JText::_('COM_SUPPORTGROUPS_IMPORT_UNABLE_TO_FIND_IMPORT_PACKAGE'));
			return false;
		}
		...
		if (!$this->setData($package,$this->dataType,$headerList))
		{
			...
			$msg = JText::_('COM_SUPPORTGROUPS_IMPORT_ERROR');
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
				$app->setUserState('com_supportgroups.redirect_url', 'index.php?option=com_supportgroups&view='.$back);
		...
		else
		{
			...
			$msg = JText::sprintf('COM_SUPPORTGROUPS_IMPORT_SUCCESS', $package['packagename']);
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
			    $app->setUserState('com_supportgroups.redirect_url', 'index.php?option=com_supportgroups&view='.$back);
	...
	**/
	protected function save($data,$table)
	{
		...
		if(SupportgroupsHelper::checkArray($data['array']))
		{
			...
			if(SupportgroupsHelper::checkArray($data['array']) && $user->authorise($table.'.import', 'com_supportgroups') && $user->authorise('core.import', 'com_supportgroups'))
			{
				// set target.
				...
				foreach($data['array'] as $row)
				{
					...
					if (isset($row[$id_key]) && is_numeric($row[$id_key]) && $row[$id_key] > 0)
					{
						...
						$query
							->select('version')
							->from($db->quoteName('#__supportgroups_'.$table))
					...
					elseif ($canCreate)
					{
						...
						$query
							->insert($db->quoteName('#__supportgroups_'.$table))
						...
						if ($done)
						{
							...
							SupportgroupsHelper::setAsset($aId,$table);
						}
					}
					else
					{
	...
	protected function getAliasesUsed($table)
	{
		...
		$columns = $db->getTableColumns('#__supportgroups_'.$table);
		if(isset($columns['alias'])){
			// Create a new query object.
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('alias')));
			$query->from($db->quoteName('#__supportgroups_'.$table));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
