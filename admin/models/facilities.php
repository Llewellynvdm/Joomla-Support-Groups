 ...
 * Facilities Model
 */
class SupportgroupsModelFacilities extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($formSubmited)
		{
			$facility_type = $app->input->post->get('facility_type');
		...
		if ($formSubmited)
		{
			$details = $app->input->post->get('details');
	...
	public function getItems()
	{
		...
		if (SupportgroupsHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('facility.access', 'com_supportgroups.facility.' . (int) $item->id) && $user->authorise('facility.access', 'com_supportgroups'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName('g.name','facility_type_name'));
		$query->join('LEFT', $db->quoteName('#__supportgroups_facility_type', 'g') . ' ON (' . $db->quoteName('a.facility_type') . ' = ' . $db->quoteName('g.id') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.facility_type LIKE '.$search.' OR g.name LIKE '.$search.' OR a.phone LIKE '.$search.' OR a.details LIKE '.$search.')');
			}
		}
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__supportgroups_facility");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__supportgroups_facility'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__supportgroups_facility'))->set($fields)->where($conditions); 
