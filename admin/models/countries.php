 ...
 * Countries Model
 */
class SupportgroupsModelCountries extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName('g.name','currency_name'));
		$query->join('LEFT', $db->quoteName('#__supportgroups_currency', 'g') . ' ON (' . $db->quoteName('a.currency') . ' = ' . $db->quoteName('g.codethree') . ')');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__supportgroups_country'))->set($fields)->where($conditions); 
