 ...
 */
class SupportgroupsModelRegion extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_supportgroups.region';
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		if ($id != 0 && (!$user->authorise('region.edit.name', 'com_supportgroups.region.' . (int) $id))
			|| ($id == 0 && !$user->authorise('region.edit.name', 'com_supportgroups')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('region.edit.country', 'com_supportgroups.region.' . (int) $id))
			|| ($id == 0 && !$user->authorise('region.edit.country', 'com_supportgroups')))
		{
			// Disable fields for display.
	...
	{
		return 'administrator/components/com_supportgroups/models/forms/region.js';
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__supportgroups_region'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_supportgroups.edit.region.data', array());
