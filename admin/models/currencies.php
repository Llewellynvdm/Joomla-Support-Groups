 ...
 * Currencies Model
 */
class SupportgroupsModelCurrencies extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($formSubmited)
		{
			$numericcode = $app->input->post->get('numericcode');
		...
		if ($formSubmited)
		{
			$decimalplace = $app->input->post->get('decimalplace');
	...
	public function getItems()
	{
		...
		if (SupportgroupsHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('currency.access', 'com_supportgroups.currency.' . (int) $item->id) && $user->authorise('currency.access', 'com_supportgroups'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__supportgroups_currency");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__supportgroups_currency'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__supportgroups_currency'))->set($fields)->where($conditions); 
