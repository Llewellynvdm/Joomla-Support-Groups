 ...
 * Info_types Model
 */
class SupportgroupsModelInfo_types extends JModelList
{
	...
	public function getItems()
	{
		...
		if (SupportgroupsHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('info_type.access', 'com_supportgroups.info_type.' . (int) $item->id) && JFactory::getUser()->authorise('info_type.access', 'com_supportgroups'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__supportgroups_info_type");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__supportgroups_info_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__supportgroups_info_type'))->set($fields)->where($conditions); 
