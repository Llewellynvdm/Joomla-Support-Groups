...
class SupportgroupsControllerCountries extends JControllerAdmin
{
	...
	public function exportData()
	{
		...
		if ($user->authorise('country.export', 'com_supportgroups') && $user->authorise('core.export', 'com_supportgroups'))
		{
			// Get the input
			...
			if (SupportgroupsHelper::checkArray($data))
			{
				...
				SupportgroupsHelper::xls($data,'Countries_'.$date->format('jS_F_Y'),'Countries exported ('.$date->format('jS F, Y').')','countries');
			}
		}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('country.import', 'com_supportgroups') && $user->authorise('core.import', 'com_supportgroups'))
		{
			...
			if (SupportgroupsHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_SUPPORTGROUPS_IMPORT_SELECT_FILE_FOR_COUNTRIES');
