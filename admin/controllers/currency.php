...
class SupportgroupsControllerCurrency extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		// Access check.
		$access = ($user->authorise('currency.access', 'com_supportgroups.currency.' . (int) $recordId) &&  $user->authorise('currency.access', 'com_supportgroups'));
		if (!$access)
		{
			return false;
		}
		...
		if ($recordId)
		{
			...
			if (!$permission)
			{
				if ($user->authorise('currency.edit.own', 'com_supportgroups.currency.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('currency.edit.own', 'com_supportgroups'))
						{
							return true;
						}
					}
