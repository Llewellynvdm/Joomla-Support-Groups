...
class SupportgroupsControllerCountry extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		// Access check.
		$access = ($user->authorise('country.access', 'com_supportgroups.country.' . (int) $recordId) &&  $user->authorise('country.access', 'com_supportgroups'));
		if (!$access)
		{
			return false;
		}
		...
		if ($recordId)
		{
			...
			if (!$permission)
			{
				if ($user->authorise('country.edit.own', 'com_supportgroups.country.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('country.edit.own', 'com_supportgroups'))
						{
							return true;
						}
					}
