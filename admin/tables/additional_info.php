 ...
 */
class SupportgroupsTableAdditional_info extends JTable
{
	/**
	...
	/**
	 * Overload the store method for the Additional_info table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New additional_info. A additional_info created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('additional_info', 'SupportgroupsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SUPPORTGROUPS_ADDITIONAL_INFO_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_supportgroups.additional_info.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_supportgroups.additional_info.'.(int) $this->$k;
	}
