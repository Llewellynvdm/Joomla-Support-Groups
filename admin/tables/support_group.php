...
/**
 * Support_groups Table class
 */
class SupportgroupsTableSupport_group extends JTable
{
	/**
	...
	{
		parent::__construct('#__supportgroups_support_group', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_supportgroups.support_group'));
	}	
	...
	/**
	 * Overload the store method for the Support_group table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New support_group. A support_group created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(JText::_('COM_SUPPORTGROUPS_SUPPORT_GROUP_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_supportgroups.support_group.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_supportgroups.support_group.'.(int) $this->$k;
	}
