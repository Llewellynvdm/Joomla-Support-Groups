	...
	<script>
		...
		function emptyTrash() {
			if (document.adminForm.boxchecked.value == 0) {
				...
				if(confirm('<?= JText::_("COM_SUPPORTGROUPS_ARE_YOU_SURE_YOU_WANT_TO_DELETE_CONFIRMING_WILL_PERMANENTLY_DELETE_THE_SELECTED_ITEMS") ?>')) {
	...
	<div class="alert alert-error">
		<?php if (empty($displayData->items)): ?>
			<h4 class="alert-heading">
				...
				<?= JText::_("COM_SUPPORTGROUPS_TRASH_AREA") ?>
			</h4>
			<p><?= JText::_("COM_SUPPORTGROUPS_YOU_ARE_CURRENTLY_VIEWING_THE_TRASH_AREA_AND_YOU_DONT_HAVE_ANY_ITEMS_IN_TRASH_AT_THE_MOMENT") ?></p>
		<?php else: ?>
			<h4 class="alert-heading">
				<span class="icon-trash"></span>
				<?= JText::_("COM_SUPPORTGROUPS_TRASHED_ITEMS") ?>
			</h4>
			<p><?= JText::_("COM_SUPPORTGROUPS_YOU_ARE_CURRENTLY_VIEWING_THE_TRASHED_ITEMS") ?></p>
			<button onclick="emptyTrash();" class="btn btn-small btn-danger">
				<span class="icon-delete" aria-hidden="true"></span>
				<?= JText::_("COM_SUPPORTGROUPS_EMPTY_TRASH") ?>
			</button>
		<?php endif; ?>
		<button onclick="exitTrash();" class="btn btn-small">
			<span class="icon-back" aria-hidden="true"></span>
			<?= JText::_("COM_SUPPORTGROUPS_EXIT_TRASH") ?>
		</button>
	</div>
<?php endif; ?>
